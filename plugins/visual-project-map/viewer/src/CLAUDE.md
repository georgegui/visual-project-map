...
2. **`viewer.js`** — `GraphViewer` IIFE (JSON loading, Cytoscape element/style building, dagre layout, validation)
3. **`diff.js`** — `GraphDiff` IIFE (structural diff between two graph JSON versions)
4. **`plan-overlay.js`** — `PlanOverlay` IIFE (plan annotation glow/badges)
...
`expand-collapse.js` must load before `viewer.js` (which instantiates `CollapseManager`), and all modules before `interactions.js` (which wires them together).
